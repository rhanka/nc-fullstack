...
- [x] L5.2q Ajouter un mode demo propose apres 15 secondes sur chat vierge. Recette: si le chat task `000` reste vide et inactif, meme widget ferme, un modal ouvre le chat et propose de demarrer avec une random non conformity; aucune generation ne part sans confirmation utilisateur. `TEST` + `UAT`
...
- Checklist UAT `L5.2` à exécuter sur un cas `000` réaliste:
  ...
  2. Si le chat reste vide pendant 15 secondes sur `000`, widget ferme inclus, vérifier que le modal demo apparaît, que `Not now` le ferme, et que `Start demo` envoie une description aléatoire.
