...
- [ ] L7.3 Sortir les données runtime de l'image API après disponibilité d'un stockage monté persistant. Recette: l'image API ne contient plus `pages/`, `json/`, `vector-export/`, `lexical/`, `ontology/` ni `wiki/`; elle devient une image applicative versionnée par le code seulement, les corpus étant lus depuis le volume runtime. `TEST`
...
- Note: ordre de priorité désormais verrouillé: `L7.1 -> L7.2 -> L7.2a -> L7.4 -> L7.5 -> L7.6 -> L7.3 -> L7.7 -> L7.8`.
- Note: `L7.3` est volontairement repoussé après `L7.5/L7.6`; sur `Serverless Containers`, sortir les données de l'image ne ferait que déplacer le coût vers une hydratation éphémère au démarrage.
